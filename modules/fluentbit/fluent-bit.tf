...
module "iam_assumable_role_with_oidc_for_fluentbit_aws" {
  ...
  oidc_fully_qualified_subjects = [
    ...
    "system:serviceaccount:${var.namespace}:${local.fluent_bit_sa_name}"
  ]
}
...
locals {
  cwfb_config_file_cm_manifest = split(
    "\n---\n",
    replace(
      replace(file("${path.module}/files/fluent-bit-config-file-cm.yaml"), "___namespace___", var.namespace),
